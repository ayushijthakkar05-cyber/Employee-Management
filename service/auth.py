...
class AuthService:
    ...
    def register_user(self, user):
        ...
        employee = (
            self.db.query(Employee).filter(Employee.email == new_user.email).first()
        )
    ...
    def login_user(self, login_data):
        ...
        access_token = create_access_token(
            {"sub": user.email, "user_uuid": str(user.uuid), "role": user.role.name}
        )
    ...
    def get_manager_dashboard(self, current_user):
        return {"message": "Welcome Manager", "user": current_user.username}

    def get_employee_dashboard(self, current_user):
        return {"message": "Welcome Employee", "user": current_user.username}

    def forgot_password(self, request):
        return forgot_password_service(self.db, request.email)

    def reset_password(self, request):
        return reset_password_service(
            self.db, request.email, request.otp, request.new_password
        )

    def resend_verification_otp(self, request):
        return resend_verification_otp_service(self.db, request.email)
